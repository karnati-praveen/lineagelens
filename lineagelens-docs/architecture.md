...
- `lineagelens-src/agentAdapters/*` — detects agent/tool context for Cursor, Claude Code, Aider, Copilot, Codeium, Continue, Cody, Amazon Q, Gemini CLI, Codex CLI, and a legacy heuristic fallback.
- `lineagelens-src/storage/*` — switches between local and backend storage services.
- `lineagelens-src/lightweightRecord.ts` — editor-free record builder for CLI, scripts, and CI jobs.
- `lineagelens-backend/app/*` — provides auth, ingest, search, explain, insights, and WebSocket capture APIs.
...
`lineagelens-src/storage/LocalStorageService.ts` keeps records in a JSON file.
...
`lineagelens-src/storage/BackendStorageService.ts` sends records to the FastAPI backend.

...
- A working `lineagelens-backend/.env` file if you want custom connection strings or secrets
...
3. For LineageLens Plus, copy `lineagelens-deploy/.env.plus.example` to `lineagelens-backend/.env` and fill in your database credentials.
4. For LineageLens Max, copy `lineagelens-deploy/.env.max.example` to `lineagelens-backend/.env` and fill in your PostgreSQL and Neo4j credentials.

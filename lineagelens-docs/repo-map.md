...
1. The capture layer activates from `lineagelens-src/extension.ts`.
2. Editor changes are correlated with proxy captures in `lineagelens-src/correlation.ts` and `lineagelens-src/proxy.ts`.
3. The extension builds a provenance record in `lineagelens-src/provenance.ts` and `lineagelens-src/eventSchema.ts`.
4. Records are stored locally through `lineagelens-src/storage/LocalStorageService.ts` or sent to the backend through `lineagelens-src/storage/BackendStorageService.ts`.
5. The backend receives ingest requests through `lineagelens-backend/app/api/routes/ingest.py` or `lineagelens-backend/app/api/routes/ws_capture.py`.
6. Search and dashboard results come from `lineagelens-backend/app/services/provenance_service.py` and `lineagelens-backend/app/services/insights_service.py`.
...
2. `lineagelens-src/extension.ts` for the capture layer entry point.
3. `lineagelens-src/eventSchema.ts` and `lineagelens-src/lightweightRecord.ts` for the shared provenance contract.
4. `lineagelens-src/storage/StorageService.ts` and the files under `lineagelens-src/storage/` for local versus backend behavior.
5. `lineagelens-backend/app/main.py` and `lineagelens-backend/app/services/ingest_normalizer.py` for backend startup and ingest normalization.
6. `lineagelens-backend/app/services/provenance_service.py` and `lineagelens-backend/app/services/insights_service.py` for storage, search, and dashboard logic.

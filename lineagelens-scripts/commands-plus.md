...
docker exec lineagelens-plus-postgres \
  ...
  -c "SELECT username, workspace_id, role, created_at FROM user_accounts ORDER BY created_at;"
...
docker exec lineagelens-plus-postgres \
  psql -U postgres -d provenance -c "SELECT username, role FROM user_accounts;"
```

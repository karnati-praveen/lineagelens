  ...
  try {
    ...
    const exportBtn = _featureAiBom
      ? `<button class="s" style="margin-left:auto;font-size:12px;padding:3px 10px" onclick="exportAiBom()">Export AI‑BOM</button>` +
        `<button id="copy-aibom-btn" class="s" style="font-size:12px;padding:3px 10px;margin-left:6px" onclick="copyAiBom()">Copy AIBOM</button>`
      : '';
    sec.innerHTML =
...
async function copyAiBom() {
  const ws = _user?.workspaceId;
  if (!ws) return;
  if (!navigator.clipboard) { alert('Clipboard API unavailable (requires HTTPS).'); return; }
  const btn = document.getElementById('copy-aibom-btn');
  try {
    const d = await req('POST', `/integrity/aibom?workspace_id=${encodeURIComponent(ws)}`);
    await navigator.clipboard.writeText(JSON.stringify(d, null, 2));
    if (btn) { btn.textContent = 'Copied!'; setTimeout(() => { btn.textContent = 'Copy AIBOM'; }, 1500); }
  } catch(e) {
    alert(`AI-BOM copy failed: ${e.message}`);
  }
}

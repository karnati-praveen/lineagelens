...
const KEY_SEEN_WELCOME = 'lineagelens.hasSeenWelcome';
...
function fireAndForgetPostLead(email: string, context: vscode.ExtensionContext): void {
    ...
    const body = JSON.stringify({ email, source: 'vscode-extension', extension_version: version });
...
function getBackendBaseUrl(): string {
    const cfg = vscode.workspace.getConfiguration('aiInsertionDetector');
    const raw = cfg.get<string>('backend.baseUrl', DEFAULT_BACKEND_BASE_URL) ?? DEFAULT_BACKEND_BASE_URL;
    return raw.replace(/\/$/, '');
}
